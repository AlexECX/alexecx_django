...
class FormsMixin(AbstractFormsMixin):
    """Barebone mixin with no <form_name> method overload."""
...
class MultiFormMixin(AbstractFormsMixin):
    ...
    def get_initials(self, form_name):
        ...
        initial_method = getattr(self, 'get_%s_initial' % form_name, None)
        if initial_method:
            return initial_method(form_name)
        else:
            initial = self.get_initial(form_name)
            return initial
    ...
    def get_prefixes(self, form_name):
        ...
        prefix_method = getattr(self, 'get_%s_prefix' % form_name, None)
        if prefix_method:
            return prefix_method(form_name)
        else:
            return self.get_prefix(form_name)

    def get_forms_kwargs(self, form_name):
        """Makes get_%_form_kwargs overload possible."""
        kwargs_method = getattr(self, 'get_%s_form_kwargs' % form_name, None)
        if kwargs_method:
            return kwargs_method(form_name)
        else:
    ...
    def forms_valid(self, forms, form_name=None):
        ...
        in a %_form_valid method and executes the first one it finds.
        ...
        # 1st name it can get
        self._form_name = next(iter(forms)) 
        return self.form_valid(forms)
